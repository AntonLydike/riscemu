...
class RV32_Xssr_pseudo(InstructionSet):
    ...
    def instruction_ssr_configure(self, ins: Instruction):
        dm = ins.get_imm(0).abs_value
        bound = ins.get_imm(1).abs_value
        stride = ins.get_imm(2).abs_value
    ...
    def instruction_ssr_read(self, ins: Instruction):
        ...
        dm = ins.get_imm(1).abs_value
        dim = ins.get_imm(2).abs_value

        self._stream.dm_by_id[dm].base = self.regs.get(base_pointer)
